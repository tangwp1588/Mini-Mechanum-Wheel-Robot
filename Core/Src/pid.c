...
void PID_Speed(void) {
    ...
    for(uint8_t i = 0; i < 4; i++) {
    	...
    	uint32_t delta = now - config->prev_tick;
		float SAMPLE_TIME = (float)delta / (float)osKernelGetTickFreq(); // in sseconds
    	config->prev_tick = now;
    	if (SAMPLE_TIME <= 0.0f || SAMPLE_TIME > 1.0f) {
    		continue;
    	}
...
void PID_Orientation(void) {
	...
	uint32_t now   = osKernelGetTickCount();
	uint32_t delta = now - config->prev_tick;
	float SAMPLE_TIME = (float)delta / (float)osKernelGetTickFreq(); // in seconds
	config->prev_tick = now;
